...
class DataProcessor(object):
    ...
    def read_preposition_senses(self):
        ...
        print("Read senses for {} prepositions.".format(num_preps), file=sys.stderr)
        print("Senses per preposition: {}".format((float(sum(num_senses_per_prep))/num_preps)), file=sys.stderr)
...
#vj                pos_tags.append(tag)
    ...
    def get_embedding_matrix(self, embedding_file, onto_aware):
        ...
        print("Found vectors for %.4f of the words".format((float(num_found_elements) / num_all_elements)), file=sys.stderr)
